...
The default local administrator is `qip-admin` with password `qip-admin-local-only`. These credentials are synthetic and development-only; copy `.env.example` to `.env` to override them.
...
The script signs in as the configured local administrator, preserves the server-managed session and CSRF token, then idempotently loads three assets and manuals. It verifies that the Atlas manual is indexed, creates a synthetic incident with one authenticated observation and one server-labelled `HUMAN_ENTERED` measurement, starts its investigation, which moves the incident to `INVESTIGATING`, before asking one document-scoped question. The observation and evidence remain visible incident records and are not sent to the model. When the answer is grounded, it proposes a draft finding, records a role-authorized review, closes the investigation with a human-authored summary, which automatically marks the incident `RESOLVED`. It then prints the incident status, observation and evidence IDs, answer, citation, finding status, audit-event count, and closure record. Each run intentionally creates a new incident so its provenance and timestamps remain honest.
...
- Human actor fields come from the authenticated principal, not caller-supplied request values; review and closure enforce explicit role boundaries.
- Closure requires no unresolved drafts and at least one confirmed finding; closed investigations are immutable.
- Successful investigation closure automatically resolves the incident in the same workflow; terminal incident `CLOSED` remains an optional later archival transition.
